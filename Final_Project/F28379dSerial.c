    ...
    if (RXCdata & 0xC000) {
        ...
        if (ESP8266insidecommands == 1) {
            ...
            if (sendingto8266 == 1) {
                ...
                if (RXCdata =='a') {
                    sendingto8266 = 1;
                    sendtoSize = sprintf(sendto8266,"%.5f%.5f\r\n",xRK,yRK);
                    if (sendtoSize <= 9) {
...
extern uint16_t is_line;
extern uint16_t line_turn;
...
__interrupt void RXDINT_recv_ready(void)
{
    ...
    if (RXDdata & 0xC000) {
        ...
        numRXD ++;
        //here you get rxd data
        if (RXDdata == 0) { //Waiting for data
            // Line is not found,
            is_line = 0;
        } else {
            is_line = 1;
            line_turn = (int)RXDdata;
        }
